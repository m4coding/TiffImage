...
static enum TIFFReadDirEntryErr TIFFReadDirEntryArray(TIFF* tif, TIFFDirEntry* direntry, uint32* count, uint32 desttypesize, void** value)
{
	if (TIFF_DEBUG) LOGI("TIFFReadDirEntryArray");
	int typesize;
	uint32 datasize;
	void* data;
...
{
	if (TIFF_DEBUG) LOGI("EstimateStripByteCounts");
...
TIFFFetchDirectory(TIFF* tif, uint64 diroff, TIFFDirEntry** pdir,
                   uint64 *nextdiroff)
{
	if (TIFF_DEBUG) LOGI("TIFFFetchDirectory");
	...
	if (!isMapped(tif)) {
		if (TIFF_DEBUG) {
			LOGI("isMapped false");
			LOGI("tif->tif_diroff==%ul",tif->tif_diroff);
		}

		toff_t hasSeek = SeekOK(tif, tif->tif_diroff);

		if (TIFF_DEBUG) LOGI("hasSeek==%u",hasSeek);
		...
		if (TIFF_DEBUG) LOGI("SeekOK true");

		if (!(tif->tif_flags&TIFF_BIGTIFF))
		{
			if (TIFF_DEBUG) LOGI("TIFF_BIGTIFF false");

			toff_t hasRead = ReadOK(tif, &dircount16, sizeof (uint16));
			//if (TIFF_DEBUG) LOGI("hasRead==%d",hasRead);
			...
			if (tif->tif_flags & TIFF_SWAB) {
				if (TIFF_DEBUG) LOGI("TIFF_SWAB true");
				TIFFSwabShort(&dircount16);
			}
			if (TIFF_DEBUG) LOGI("dircount16==%d", dircount16);
			if (dircount16>4096)
			{
				TIFFErrorExt(tif->tif_clientdata, module,
			...
			if (TIFF_DEBUG) LOGI("TIFF_BIGTIFF true");
		...
		if (origdir == NULL) {
			if (TIFF_DEBUG) LOGI("=======_TIFFCheckMalloc origdir==0");
			return 0;
		}
		...
		if (TIFF_DEBUG) LOGI("isMapped true");
		tmsize_t m;
...
{
	if (TIFF_DEBUG) LOGI("TIFFFetchStripThing");
...
{
	if (TIFF_DEBUG) LOGI("ChopUpSingleUncompressedStrip");
